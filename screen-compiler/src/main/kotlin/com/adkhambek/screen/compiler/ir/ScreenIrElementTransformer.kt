...
// Import classId to get the ClassId (package + relative class name) of an IR class.
...
// Returns the JVM binary name for a class, using '$' as the nested-class separator.
// kotlinFqName.asString() uses dots for nested classes (e.g. "com.example.Outer.Inner"),
// but Class.forName() and FragmentFactory.instantiate() require dollar signs ("com.example.Outer$Inner").
private fun IrClass.jvmBinaryName(): String {
    val id = classId ?: return kotlinFqName.asString()
    val pkg = id.packageFqName.asString()
    val relative = id.relativeClassName.asString().replace('.', '$')
    return if (pkg.isEmpty()) relative else "$pkg.$relative"
}
...
class ScreenIrElementTransformer(
    ...
    private fun generateArgGetter(declaration: IrProperty, parentClass: IrClass) {
        ...
        // Get the JVM binary name of the arg class for Class.forName() and the parent class FQN for the Bundle key.
        val argClassFqn = returnType.classOrNull!!.owner.jvmBinaryName()
    ...
    override fun visitSimpleFunction(declaration: IrSimpleFunction): IrStatement {
        ...
        // Get the JVM binary name of the outer class for use in Class.forName() and factory.instantiate().
        val fqName = outerClass.jvmBinaryName()
